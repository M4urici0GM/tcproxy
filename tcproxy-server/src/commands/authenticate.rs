...
impl AsyncCommand for AuthenticateCommand {
    ...
    async fn handle(&mut self) -> Self::Output {
        if self.auth_manager.is_authenticated() {
            send_authentication_failed_frame(&self.client_sender, &Reason::AuthenticationFailed)
                .await?;
            return Ok(());
        }
        ...
        let (user, token) = match self.authenticate().await {
            ...
            Err(AuthenticateCommandError::AuthenticationFailed) => {
                send_authentication_failed_frame(
                    &self.client_sender,
        ...
        let authenticate_ack =
            AuthenticateAck::new(user.id().to_string().as_ref(), user.email(), token);
